...
class Trainer:
    ...
    def __init__(
        ...
        Args:
            model: PyTorch model to train (should have forward(theta, world_id) -> IRF)
        ...
        # Horizon weights for multi-horizon loss
        self.horizon_weights = self._create_horizon_weights()
    ...
    def compute_loss(self, batch: dict[str, Any], predictions: torch.Tensor) -> torch.Tensor:
    ...
    def train_epoch(self) -> dict[str, float]:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            # Forward pass - handle single world vs mixed world
            if len(set(world_ids)) == 1:
                # Single world batch
                predictions = self.model(theta, world_ids[0])
            else:
                # Mixed world batch - need to handle per-world
                # For now, assume model can handle batches with world_id list
